...
class historicModel {
    ...
    public function getOrderHistoryByUserId($user_id) {
        $sql = "SELECT o.order_id, o.product_id, o.quantity, o.price, od.name, od.email, od.phone, od.address, od.note, od.order_detail_id
                FROM orders o
    ...
    public function getOrderDetails($order_detail_id) {
        $sql = "SELECT 
                ...
                WHERE o.order_detail_id = :order_detail_id"; // Sửa WHERE để tìm theo order_detail_id
        $stmt = $this->conn->prepare($sql);
